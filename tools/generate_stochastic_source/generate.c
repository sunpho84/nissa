...
int main(int narg,char **arg)
{
  ...
  for(int loc_site=0;loc_site<loc_vol;loc_site++)
	...
	{
	  if((take_slice==0 || glb_coord_of_loclx[loc_site][0]==twall))
	    {
	      switch(noise_type)
		{
		case -1:
		  spinore[loc_site][id][ic1][0]=-1;
		  spinore[loc_site][id][ic1][1]=0;
		  break;
		case +1:
		  spinore[loc_site][id][ic1][0]=+1;
		  spinore[loc_site][id][ic1][1]=0;
		  break;
		case +2:
		  spinore[loc_site][id][ic1][0]=pm_one(loc_site)/rad2;
		  spinore[loc_site][id][ic1][1]=0;
		  break;
		case +4:
		  spinore[loc_site][id][ic1][0]=pm_one(loc_site)/rad2;
		  spinore[loc_site][id][ic1][1]=pm_one(loc_site)/rad2;
		  break;
		default:
		  if(rank==0)
		    {
