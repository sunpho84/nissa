...
int compute_allocable_propagators(int nprop_list,int nch_contr)
{
  ...
  su3spinspin *fuf=NULL;
  int nmin_req;
  if(nch_contr==0) nmin_req=2;
  else nmin_req=3;
  fuf=(su3spinspin*)malloc(nmin_req*sizeof(su3spinspin)*loc_vol);
  ...
  do
    {
      nprop_max--;
      fuf=(su3spinspin*)malloc((nprop_max+1)*sizeof(su3spinspin)*loc_vol);
    }
  while(fuf==NULL);
...
void meson_two_points(complex *corr,int *list_op1,su3spinspin *s1,int *list_op2,su3spinspin *s2,int ncontr,int f1,int r1,int f2,int r2)
{
...
int main(int narg,char **arg)
{
  ...
  su3spinspin **spinor1=nissa_malloc("spinor1*",nprop_per_block,su3spinspin*);
  for(int iprop1=0;iprop1<nprop_per_block;iprop1++) spinor1[iprop1]=nissa_malloc("spinor1",loc_vol,su3spinspin);
  su3spinspin *spinor2=nissa_malloc("spinor2",loc_vol,su3spinspin);
  ...
  su3spinspin *ch_spinor=NULL;
  as2t_su3 *Pmunu=NULL;
  quad_su3 *gauge_conf;
  if(nch_contr!=0)
    {
      ch_spinor=nissa_malloc("ch_spinor",loc_vol,su3spinspin);
  ...
  for(int iblock=0;iblock<nblocks;iblock++)
	...
	read_su3spinspin(spinor1[iprop1],base_filename1[counter],NULL);
	tot_reading_time+=take_time();
	tot_prop_read++;
      }
      ...
      for(int iprop2=0;iprop2<nprop_list2;iprop2++)
	{
	  ...
	  su3spinspin *spinor2_ptr; //This will point to spinor2 if the prop. is not in the first list
	  ...
	  if(spinor2_ptr==spinor2)
	    {
	      ...
	      tot_reading_time-=take_time();
	      read_su3spinspin(spinor2,base_filename2[iprop2],NULL);
	      tot_reading_time+=take_time();
	      tot_prop_read++;
	    }
	  
	  //apply the chromo magnetic operator to the second spinor
	  if(nch_contr>0) unsafe_apply_chromo_operator_to_su3spinspin(ch_spinor,Pmunu,spinor2_ptr);
