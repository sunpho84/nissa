...
#            rm input
  ...
  echo "GaugeConf ../../Conf" >> $base_Bk/input
  echo "ThetaTXYZ 1 0 0 0" >> $base_Bk/input
  echo "Kappa "$kappa >> $base_Bk/input
  for((ilist=0;ilist<2;ilist++))
  do
    echo "NProp"${name_list[$ilist]}"List "$((${nmu_list[$ilist]})) >> $base_Bk/input
    for((imu=0;imu<${nmu_list[$ilist]};imu++))
    do
      mu=${list_mu[$imu]}
      echo " "$base_conf/Props/$is_source_name\_0/$mu/prop fuf $base_conf/Props/$is_source_name\_1/$mu/prop fuf $mu >> $base_Bk/input
    done
  done
  ...
  $MPI_TM_PREF $base_nissa/Appretto/projects/eight_BK/eight_BK_reco input
