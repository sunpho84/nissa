/***********************************************************************
 * $Id: operator.c 1553 2010-07-23 09:13:16Z urbach $ 
 *
...
void op_write_prop(const int op_id, const int index_start, const int append_) {
  ...
  if (PropInfo.format == 1) {
    ...
    if(optr->no_flavours == 2) {
      write_spinor(writer, &operator_list[op_id].sr2, &operator_list[op_id].sr3, 1, 32);
    }
    free(sourceFormat);
  }
  ...
  if(optr->no_flavours == 2) {
    write_spinor(writer, &operator_list[op_id].prop2, &operator_list[op_id].prop3, 1, 32);
  }
  write_spinor(writer, &operator_list[op_id].prop0, &operator_list[op_id].prop1, 1, 32);
