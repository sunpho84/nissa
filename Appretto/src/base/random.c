...
void start_loc_rnd_gen(int seed)
{
  ...
  if(loc_vol==0) crash("Error, grid not initalized\n");
  ...
  loc_rnd_gen=appretto_malloc("Loc_rnd_gen",loc_vol,rnd_gen);
  for(int ivol=0;ivol<loc_vol;ivol++) start_rnd_gen(&(loc_rnd_gen[ivol]),seed+glblx_of_loclx[ivol]);
  
  loc_rnd_gen_inited=1;
  master_printf("\nGrid of local random generators initialized\n");
}

//Stop grid of local random generators
void stop_loc_rnd_gen()
{
  master_printf("\nStopping local random generators\n");
  if(loc_rnd_gen_inited==1)
    {
      appretto_free(loc_rnd_gen);
