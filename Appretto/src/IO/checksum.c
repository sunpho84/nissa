...
{
  if(big_endian) return crc32(crc,buf,len);
  else
    {
      double temp_buf[len/8];
      doubles_to_doubles_changing_endianess(temp_buf,(double*)buf,len/8);
	...
	for(x[3]=0;x[3]<loc_size[3];x[3]++)
	  {
	    ...
	    uint32_t ivol=     x[1]+loc_size[1]*(x[2]+loc_size[2]*(x[3]+loc_size[3]*x[0]));
