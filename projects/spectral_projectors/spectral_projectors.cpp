...
void read_sp_meas_pars(spectral_proj_meas_pars_t &pars)
{
  pars.path=read_path();
  read_smooth_pars(pars.smooth_pars,true);
}
...
void in_main(int narg,char **arg)
{
  ...
  //read topo pars
  spectral_proj_meas_pars_t sp_meas_pars;
  read_str_int("MeasureSpectProj",&flag);
  if(flag)
    read_sp_meas_pars(sp_meas_pars);
  ...
  measure_topology_lx_conf_sp(sp_meas_pars,conf,0,0,false);
