...
void initialize_semileptonic(char *input_path)
{
  ...
  for(int iprop=0;iprop<npropS0;iprop++)
    {
      S0[0][iprop]=nissa_malloc("S0[0][iprop]",loc_vol,colorspinspin);
      S0[1][iprop]=nissa_malloc("S0[1][iprop]",loc_vol,colorspinspin);
    }
  ...
  original_source=nissa_malloc("orig_source",loc_vol,colorspinspin);
}
