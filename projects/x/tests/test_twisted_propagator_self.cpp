...
int main(int narg,char **arg)
{
  ...
  compute_self_energy_twisted_diagram_in_x_space(self,quark,gluon);
  pass_spinspin_from_x_to_mom_space(self,self,quark.bc);

  FILE *fout=open_file("self","w");
  NISSA_LOC_VOL_LOOP(imom)
      ...
      for(int mu=0;mu<4;mu++)
	{
      ...
      if(a2p2<=4)
	master_fprintf(fout,"%lg\t%lg\n",a2p2,glb_vol*real_part_of_trace_with_igamma(self,imom));
    }
