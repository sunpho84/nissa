...
namespace nissa
{
  ...
  int read_conf_parameters(int &iconf,bool(*external_condition)())
  {
      ...
      do
	{
	  ...
	  char run_file[1024];
	  if(snprintf(run_file,1024,"%s/%s",outfolder, running_filename.c_str())<0) crash("witing %s",run_file);
  ...
  {
    char fin_file[1024];
    if(snprintf(fin_file,1024,"%s/%s",outfolder, finished_filename.c_str())<0) crash("writing %s",fin_file);
    file_touch(fin_file);
    nanalyzed_conf++;
  }
