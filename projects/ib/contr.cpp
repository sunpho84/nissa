...
namespace nissa
{
  ...
  {
    mes2pts_contr_size=glb_size[0]*mes_gamma_list.size()*mes2pts_contr_ins_map.size()*mes2pts_contr_quark_map.size();
  ...
  {
    mes2pts_contr_ins_map.clear();
    
    //non-perturbed
    mes2pts_contr_ins_map.push_back(mes_doublet_t(PROP_0,PROP_0));
    
    //mass corrections
    if(compute_mass_corrections) mes2pts_contr_ins_map.push_back(mes_doublet_t(PROP_0,PROP_S));
    
    //QED corrections
    if(compute_QED_corrections)
      {
	mes2pts_contr_ins_map.push_back(mes_doublet_t(PROP_0,PROP_P));
	mes2pts_contr_ins_map.push_back(mes_doublet_t(PROP_0,PROP_T));
	mes2pts_contr_ins_map.push_back(mes_doublet_t(PROP_0,PROP_PHOTON_AB));
	mes2pts_contr_ins_map.push_back(mes_doublet_t(PROP_PHOTON_A,PROP_PHOTON_B));
      }
    //mes2pts_contr_ins_map.push_back(mes_doublet_t(PROP_0,PROP_VECTOR));
  }
  ...
  THREADABLE_FUNCTION_0ARG(compute_mes2pts_contr)
  {
    ...
    for(size_t ins_combo=0;ins_combo<mes2pts_contr_ins_map.size();ins_combo++)
      for(size_t iq_combo=0;iq_combo<mes2pts_contr_quark_map.size();iq_combo++)
	for(size_t ihadr_contr=0;ihadr_contr<mes_gamma_list.size();ihadr_contr++)
	  {
		...
		for(int b=0;b<nso_col;b++)
		  {
		    int ip1=iqprop(iq1,mes2pts_contr_ins_map[ins_combo].a,j,b);
		    int ip2=iqprop(iq2,mes2pts_contr_ins_map[ins_combo].b,i,b);
			...
			NISSA_PARALLEL_LOOP(ivol,0,loc_vol)
			  {
			    ...
			    complex_summ_the_prod(loc_contr[ind_mes2pts_contr(ins_combo,iq_combo,ihadr_contr,glb_coord_of_loclx[ivol][0])],c,AB);
			  }
		      }
		  }
	      }
    ...
    if(IS_MASTER_THREAD)
      {
	nmes2pts_contr+=mes2pts_contr_ins_map.size()*mes2pts_contr_quark_map.size()*mes_gamma_list.size();
  ...
  void print_mes2pts_contr()
  {
    ...
    for(size_t ins_combo=0;ins_combo<mes2pts_contr_ins_map.size();ins_combo++)
      {
	FILE *fout=open_file(combine("%s/mes_contr_%c%c",outfolder,qprop_list[mes2pts_contr_ins_map[ins_combo].a].shortname,qprop_list[mes2pts_contr_ins_map[ins_combo].b].shortname).c_str(),"w");
  ...
  THREADABLE_FUNCTION_6ARG(meson_part_leptonic_contr, spinspin*,hadr, int,iq1, int,prop1_type, int,iq2, int,prop2_type, int,irev)
  {
	...
	{
	  int ip1=iqprop(iq1,prop1_type,id_so,ic_so);
	  int ip2=iqprop(iq2,prop2_type,id_so,ic_so);
  ...
  */
  ...
  {
    bar2pts_contr_size=ind_bar2pts_contr(bar2pts_contr_ins_map.size()-1,nsm_sink-1,bar2pts_contr_quark_map.size()-1,2-1,glb_size[0]-1)+1;
  ...
  {
    //clear the list
    bar2pts_contr_ins_map.clear();
    
    //non-perturbed
    bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_0,PROP_0,PROP_0));
    
    //mass corrections
    if(compute_mass_corrections)
      {
	//scalar insertion on one of the three lines
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_S,PROP_0,PROP_0));
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_0,PROP_S,PROP_0));
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_0,PROP_0,PROP_S));
      }
	...
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_P,PROP_0,PROP_0));
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_0,PROP_P,PROP_0));
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_0,PROP_0,PROP_P));
	//tadpole insertion on one of the three lines
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_T,PROP_0,PROP_0));
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_0,PROP_T,PROP_0));
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_0,PROP_0,PROP_T));
	//self-energy insertion on one of the three lines
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_PHOTON_AB,PROP_0,PROP_0));
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_0,PROP_PHOTON_AB,PROP_0));
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_0,PROP_0,PROP_PHOTON_AB));
	//photon exchange between one of the three lines
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_0,PROP_PHOTON_A,PROP_PHOTON_B));
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_PHOTON_A,PROP_0,PROP_PHOTON_B));
	bar2pts_contr_ins_map.push_back(bar_triplet_t(PROP_PHOTON_A,PROP_PHOTON_B,PROP_0));
      }
  }
