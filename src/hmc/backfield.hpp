...
namespace nissa
{
  ...
  void add_em_field_to_backfield(eo_ptr<quad_u1> S,quark_content_t *quark_content,double em_str,int q,const Dir& mu,const Dir& nu);
  void add_em_field_to_backfield(eo_ptr<quad_u1> S,quark_content_t *quark_content,em_field_pars_t &em_field_pars);
  
  CUDA_MANAGED extern void (*get_args_of_quantization[3])(GlbCoords& phase,const LocLxSite& ivol,const Dir& mu,const Dir& nu);
}
#endif
