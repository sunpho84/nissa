...
namespace nissa
{
  ...
  void set_lx_geometry()
  {
	...
	locd_perp_size_per_dir[mu]=(int)std::min((int64_t)max_locd_perp_size_per_dir[mu],glb_vol/glb_size[mu]-max_locd_perp_size_per_dir[mu]*rank);
	verbosity_lv3_master_printf("rank %d locd_perp_size_per_dir[%d]: %d\n",rank,mu,locd_perp_size_per_dir[mu]);
	locd_size_per_dir[mu]=locd_perp_size_per_dir[mu]*glb_size[mu];
  ...
  CUDA_HOST_AND_DEVICE void get_stagphase_of_lx(coords ph,int ivol)
  {
    ...
    for(int mu=1;mu<NDIM;mu++)
      ph[mu]=ph[mu-1]*(1-2*(glb_coord_of_loclx[ivol][mu-1]%2));
  }
