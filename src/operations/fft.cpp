...
namespace nissa
{
  ...
  void data_coordinate_order_shift(complex *data,int ncpp,const Dir& mu0)
  {
	...
	pos[in.nastyConvert()]=out.nastyConvert();
      }
  ...
  void fft1d(complex *out,complex *in,int ncpp,const Dir& mu,double sign,int normalize)
  {
    ...
    complex *buf=nissa_malloc("buf",locSize(mu).nastyConvert()*ncpp,complex);
    
    if(IS_MASTER_THREAD)
      {
	const int log2glb_nblk=find_max_pow2(glbSize(mu)()); //number of powers of 2 contained in n
	const int glb_nblk=1<<log2glb_nblk;                 //remaining odd block
	const int blk_size=glbSize(mu)()/glb_nblk;           //block size
	const int loc_nblk=glb_nblk/nrank_dir(mu)();          //number of local blocks
	...
	for(int loc_iblk_in=0;loc_iblk_in<loc_nblk;loc_iblk_in++)
	  for(int iel_blk_in=0;iel_blk_in<blk_size;iel_blk_in++)
	    {
	      //find the full index
	      const int loc_iel_in=loc_iblk_in*blk_size+iel_blk_in;
	      const int glb_iel_in=glbCoordOfLoclx(LocLxSite(0),mu)()+loc_iel_in;
	      const int glb_iblk_in=rank_coord(mu)()*loc_nblk+loc_iblk_in;
	      
	      //look at the output
	      const int iel_blk_out=glb_iel_in/glb_nblk;
	      const int glb_iblk_out=bitrev(glb_iel_in-iel_blk_out*glb_nblk,log2glb_nblk);
	      const int rank_coord_out=glb_iblk_out/loc_nblk;
	      const int loc_iblk_out=glb_iblk_out-rank_coord_out*loc_nblk;
	      const int loc_iel_out=loc_iblk_out*blk_size+iel_blk_out;
		...
		MPI_Isend((void*)(in+loc_iel_in*ncpp),ncpp*2,MPI_DOUBLE,rank_coord_out,1241+loc_iel_out,
			  line_comm[mu.nastyConvert()],&request0[nrequest0++]);
	...
	if(blk_size==1) memcpy(out,buf,locSize(mu)()*ncpp*sizeof(complex));
	else
	  {
	    ...
	    memset(out,0,locSize(mu).nastyConvert()*ncpp*sizeof(complex));
  ...
  void fft4d(complex* out,complex* in,const Coords<bool>& ext_dirs,int ncpp,double sign,int normalize)
  {
	...
	if(IS_MASTER_THREAD)
	  for(int idir=0;idir<ndirs;idir++)
	    {
	      n_per_dir[idir]=glbSize(list_dirs[idir])();
	      plans[idir]=fftw_plan_many_dft(1,&n_per_dir[idir],ncpp,buf,NULL,ncpp,1,buf,NULL,ncpp,1,sign,FFTW_ESTIMATE);
	    }
	THREAD_BARRIER();
