	...
	while(true_precision>=required_precision && precision>=required_precision)
	  {
		...
		NISSA_PARALLEL_LOOP(ivol,0,loc_vol)
		  if(loclx_parity[ivol]==par)
		      ...
		      for(int mu=0;mu<4;mu++)
			{
			  ...
			  int b=loclx_neighdw[ivol][mu];
			  if(f>=loc_vol) su3_copy(((su3*)send_buf)[loceo_of_loclx[f]-loc_volh],w_conf[ivol][mu]);
			  if(b>=loc_vol) su3_copy(((su3*)send_buf)[loceo_of_loclx[b]-loc_volh],w_conf[b][mu]);
			}
		    }
		THREAD_BARRIER();
		...
		NISSA_PARALLEL_LOOP(ivol,0,loc_vol)
		  if(loclx_parity[ivol]!=par)
			...
			int f=loclx_neighup[ivol][mu];
			int b=loclx_neighdw[ivol][mu];
			if(f>=loc_vol) su3_copy(w_conf[ivol][mu],((su3*)recv_buf)[loceo_of_loclx[f]-loc_volh]);
			if(b>=loc_vol) su3_copy(w_conf[b][mu],((su3*)recv_buf)[loceo_of_loclx[b]-loc_volh]);
		      }
		THREAD_BARRIER();
