...
namespace nissa
{
  struct idirac_pair_t
  {
    int so;
    int si;
    idirac_pair_t(int so,int si) : so(so),si(si) {}
  };
  ...
  void trace_g_ccss_dag_g_ccss(complex *c,complex *l,dirac_matr *gso,su3spinspin *s1,dirac_matr *gsi,su3spinspin *s2,int ncontr);
