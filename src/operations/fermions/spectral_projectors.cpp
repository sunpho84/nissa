...
namespace nissa{
  ...
  THREADABLE_FUNCTION_6ARG(fill_eigenpart, complex**,eigvec, quad_su3**,conf,double*, charge_cut, double*, DD_reigs,int,neigs, double,eig_precision)
  {
//    GET_THREAD_ID();
...
//    //wrap the application of DD^+ into an object that can be passed to the eigenfinder
    ...
    color *fill_tmp_eo[2]={nissa_malloc("fill_tmp_EVN",loc_volh+bord_volh,color),nissa_malloc("fill_tmp_ODD",loc_volh+bord_volh,color)};
		...
		quad_u1 *u1b[2];
    u1b[0]=nissa_malloc("u1b",loc_vol+bord_vol,quad_u1);
    u1b[1]=nissa_malloc("u1b",loc_vol+bord_vol,quad_u1);
    init_backfield_to_id(u1b);
    const auto imp_mat=[conf,in_tmp_eo,out_tmp_eo](complex *out_lx,complex *in_lx){
      split_lx_vector_into_eo_parts((color**)in_tmp_eo,(color*)in_lx);
      ...
      paste_eo_parts_into_lx_vector((color*)out_lx,(color**)in_tmp_eo);
...
//    double norm_cut[neigs];
    complex *point_vec = nissa_malloc("point_vec",loc_vol+bord_vol,complex);
    ...
    for(int ieig=0; ieig<neigs; ++ieig){
      ...
      stag::summ_the_trace(&charge_cut[ieig],point_vec,out_tmp_eo,fill_tmp_eo);
...
//      master_printf("charge_cut[%d] = %.10f, norm_cut[%d] = %.10f\n",ieig,charge_cut[ieig],ieig,norm_cut[ieig]);
      ...
      DD_reigs[ieig]=DD_eig_val[ieig][RE];
    }
    master_printf("\n\n\n");
  ...
  void measure_spectral_proj(quad_su3 **conf,theory_pars_t &theory_pars,spectr_proj_meas_pars_t &meas_pars, int iconf,bool conf_created)
  {
    ...
    double *DD_reigs =nissa_malloc("DD_reigs",neigs,double);
    ...
    for(int hit=0;hit<nhits;hit++)
    {
      ...
      fill_eigenpart(eigvec,conf,charge_cut,DD_reigs,meas_pars.neigs,meas_pars.eig_precision);
       
    }
    master_fprintf(file,"%d\t",neigs); 
    for(int ieig=0; ieig<neigs; ++ieig){
      master_fprintf(file,"%.16lg\t%.16lg\t",DD_reigs[ieig]-0.0001,charge_cut[ieig]);
    }
    master_fprintf(file,"\n");
