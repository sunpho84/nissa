...
namespace nissa
{
  ...
  void remap_vector(char *out,char *in,coords *xto,coords *xfr,int bps)
  {
    ...
    int *nfr=nissa_malloc("nfr",nranks,int);
    int *nto=nissa_malloc("nto",nranks,int);
    
    //reset the count of data from each node
    memset(nfr,0,sizeof(int)*nranks);
    memset(nto,0,sizeof(int)*nranks);
    ...
    NISSA_LOC_VOL_LOOP(ivol)
    {
      ...
      if(rank_to[ivol]>=nranks||rank_to[ivol]<0)
    ...
    char **out_buf_rank=nissa_malloc("out_buf_rank",nranks,char*);
	...
	if(rank!=irank)
	  {
	    ...
	    if(nfr[irank]!=0)
	      MPI_Irecv(recv_buf,nfr[irank]*(bps+sizeof(int)),MPI_CHAR,irank,909+irank*nranks+rank,cart_comm,&req_list[ireq++]);
	    if(nto[irank]!=0)
	      MPI_Isend(send_buf,nto[irank]*(bps+sizeof(int)),MPI_CHAR,irank,909+rank*nranks+irank,cart_comm,&req_list[ireq++]);
	  }
	else
	  {
