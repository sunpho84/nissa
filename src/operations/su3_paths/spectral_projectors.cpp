...
namespace nissa
{
  ...
  */
  void four_leaves_point_sp(as2t_su3 leaves_summ,quad_su3 *conf,int X)
  {
  ...
  THREADABLE_FUNCTION_2ARG(four_leaves_sp, as2t_su3*,leaves_summ, quad_su3*,conf)
  {
    GET_THREAD_ID();
    communicate_lx_quad_su3_edges(conf);
    NISSA_PARALLEL_LOOP(ivol,0,loc_vol) four_leaves_point_sp(leaves_summ[ivol],conf,ivol);
    set_borders_invalid(leaves_summ);
  }
  THREADABLE_FUNCTION_END
  
  //measure the topological charge site by site
  THREADABLE_FUNCTION_2ARG(local_topological_charge_sp, double*,charge, quad_su3*,conf)
  {
  ...
  THREADABLE_FUNCTION_2ARG(total_topological_charge_lx_conf_sp, double*,tot_charge, quad_su3*,conf)
  {
    double *charge=nissa_malloc("charge",loc_vol,double);
    local_topological_charge_sp(charge,conf);
  ...
  THREADABLE_FUNCTION_2ARG(total_topological_charge_eo_conf_sp, double*,tot_charge, quad_su3**,eo_conf)
  {
    //convert to lx
    quad_su3 *lx_conf=nissa_malloc("lx_conf",loc_vol+bord_vol+edge_vol,quad_su3);
    paste_eo_parts_into_lx_vector(lx_conf,eo_conf);
    
    total_topological_charge_lx_conf_sp(tot_charge,lx_conf);
  ...
  THREADABLE_FUNCTION_1ARG(compute_topo_corr_sp, double*,charge)
  {
    GET_THREAD_ID();
  ...
  int index_to_topo_corr_remapping_sp(int iloc_lx)
  {
  ...
  //wrapper
  void index_to_topo_corr_remapping_sp(int &irank,int &iloc,int iloc_lx,void *pars)
  {
    int iglb=index_to_topo_corr_remapping_sp(iloc_lx);
  ...
  void store_topo_corr_sp(FILE *file,double *corr,int itraj,double top,vector_remap_t *topo_corr_rem)
  {
  ...
  void measure_topology_lx_conf_sp(spectral_proj_meas_pars_t &pars,quad_su3 *unsmoothed_conf,int iconf,bool conf_created,bool preserve_unsmoothed)
  {
	...
	topo_corr_rem=new vector_remap_t(loc_vol,index_to_topo_corr_remapping_sp,NULL);
      }
	...
	local_topological_charge_sp(charge,smoothed_conf);
	//total charge
	double tot_charge;
	double_vector_glb_collapse(&tot_charge,charge,loc_vol);
	total_topological_charge_lx_conf_sp(&tot_charge,smoothed_conf);
	...
	if(pars.meas_corr)
	  {
	    compute_topo_corr_sp(charge);
	    store_topo_corr_sp(corr_file,charge,iconf,tot_charge,topo_corr_rem);
	  }
      }
    while(!finished);
  ...
  void measure_topology_eo_conf_sp(spectral_proj_meas_pars_t &pars,quad_su3 **unsmoothed_conf_eo,int iconf,bool conf_created)
  {
    ...
    measure_topology_lx_conf_sp(pars,unsmoothed_conf_lx,iconf,conf_created,false);
    nissa_free(unsmoothed_conf_lx);
  }
  
  //compute the topological staples site by site
  THREADABLE_FUNCTION_2ARG(topological_staples_sp, quad_su3*,staples, quad_su3*,conf)
  {
    GET_THREAD_ID();
  ...
  void topotential_pars_t::store_if_needed_sp(quad_su3 **ext_conf,int iconf)
  {
	...
	//compute topocharge
	total_topological_charge_eo_conf_sp(&charge,conf);
	master_printf("Spectral topological charge to be stored: %lg\n",charge);
	update(iconf,charge);
  ...
  //print pars
  std::string spectral_proj_meas_pars_t::get_str(bool full)
  {
