...
namespace nissa
{
  ...
  //reduce a double
  double glb_reduce_double(double in_loc,double (*thread_op)(const double&,const double&),MPI_Op mpi_op)
  {
    double out_glb;
	...
	double* glb_double_reduction_buf=get_reducing_buffer<double>(nthreads);
  ...
  float glb_reduce_single(float in_loc)
  {
	...
	float* glb_single_reduction_buf=get_reducing_buffer<float>(nthreads);
  ...
  void glb_reduce_float_128(float_128 out_glb,float_128 in_loc)
  {
	...
	float_128* glb_quadruple_reduction_buf=get_reducing_buffer<float_128>(nthreads);
	...
	if(IS_MASTER_THREAD)
	  {
	    ...
	    MPI_Allreduce(&in_loc[0],&glb_quadruple_reduction_buf[0][0],1,MPI_FLOAT_128,MPI_FLOAT_128_SUM,MPI_COMM_WORLD);
...
#endif
      MPI_Allreduce(&in_loc[0],&out_glb[0],1,MPI_FLOAT_128,MPI_FLOAT_128_SUM,MPI_COMM_WORLD);
  }
