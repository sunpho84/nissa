...
void set_buffered_comm(buffered_comm_t &in)
{
  ...
  if(Kernel_CreateMemoryRegion(&mem_region,in.recv_buf,in.tot_mess_size)) crash("creating memory region");
...
void set_lx_or_eo_buffered_comm(buffered_comm_t &in,int nbytes_per_site,int lx_eo)
{
  ...
  in.tot_mess_size=nbytes_per_site*bord_vol/div_coeff;
  ...
  for(int bf=0;bf<2;bf++) 
	...
	//set the parameters
	in.send_offset[idir]=(bord_offset[mu]+bord_vol/2*(!bf))*nbytes_per_site/div_coeff;
	in.message_length[idir]=bord_dir_vol[mu]*nbytes_per_site/div_coeff;
	in.recv_offset[idir]=(bord_offset[mu]+bord_vol/2*bf)*nbytes_per_site/div_coeff;
#ifdef BGQ
...
void fill_buffered_sending_buf_with_lx_vec(buffered_comm_t *a,void *vec,size_t nbytes_per_site)
{
  ...
  if(a->tot_mess_size!=nbytes_per_site*bord_vol)
    crash("wrong buffer size (%d) for %d large border)",a->tot_mess_size,nbytes_per_site*bord_vol);
...
void fill_lx_bord_with_buffered_receiving_buf(void *vec,buffered_comm_t *a,size_t nbytes_per_site)
{
  ...
  if(IS_MASTER_THREAD)
	...
	crash("wrong buffer size (%d) for %d large border)",a->tot_mess_size,nbytes_per_site*bord_vol);
      ...
      memcpy((char*)vec+loc_vol*nbytes_per_site,nissa_recv_buf,a->tot_mess_size);
    }
  ...
  if(!check_borders_valid(vec))
    {
      ...
      fill_buffered_sending_buf_with_lx_vec(a,vec,nbytes_per_site);
  ...
  if(!check_borders_valid(vec) && (a->nrequest!=0))
    {
      ...
      fill_lx_bord_with_buffered_receiving_buf(vec,a,nbytes_per_site);
      if(IS_MASTER_THREAD) tot_nissa_comm_time+=take_time();
  ...
  if(!check_borders_valid(vec))
    {
      ...
      start_buffered_communicating_lx_borders(a,vec,nbytes_per_site);
      finish_buffered_communicating_lx_borders(vec,a,nbytes_per_site);
    }
}
...
void fill_buffered_sending_buf_with_ev_or_od_vec(buffered_comm_t *a,void *vec,size_t nbytes_per_site,int eo)
{
  ...
  //check buffer size matching
  if(a->tot_mess_size!=nbytes_per_site*bord_volh) crash("wrong buffer size (%d) for %d border)",a->tot_mess_size,nbytes_per_site*bord_volh);
  
  //copy one by one the surface of vec inside the sending buffer
  NISSA_PARALLEL_LOOP(ibord,0,bord_volh)
    memcpy(nissa_send_buf+ibord*nbytes_per_site,(char*)vec+surfeo_of_bordeo[eo][ibord]*nbytes_per_site,nbytes_per_site);
  ...
  if(IS_MASTER_THREAD)
    {
      ...
      //check buffer size matching
      if(a->tot_mess_size!=nbytes_per_site*bord_volh)
	crash("wrong buffer size (%d) for %d border)",a->tot_mess_size,nbytes_per_site*bord_volh);
      
      //the buffer is already ordered as the vec border
      memcpy((char*)vec+loc_volh*nbytes_per_site,nissa_recv_buf,a->tot_mess_size);
    }
  ...
  if(!check_borders_valid(vec))
    {
      ...
      //fill the communicator buffer, start the communication and take time
      fill_buffered_sending_buf_with_ev_or_od_vec(a,vec,nbytes_per_site,eo);
      buffered_comm_start(a);
      if(IS_MASTER_THREAD) tot_nissa_comm_time+=take_time();
    }
}
  ...
  if(!check_borders_valid(vec) && a->nrequest!=0)
    {
      ...
      //wait communication to finish, fill back the vector and take time
      buffered_comm_wait(a);
      fill_ev_or_od_bord_with_buffered_receiving_buf(vec,a,nbytes_per_site);
      if(IS_MASTER_THREAD) tot_nissa_comm_time+=take_time();
  ...
  if(!check_borders_valid(vec))
    {
      ...
      start_buffered_communicating_ev_or_od_borders(a,vec,nbytes_per_site,eo);
      finish_buffered_communicating_ev_or_od_borders(vec,a,nbytes_per_site);
    }  
}
...
void fill_buffered_sending_buf_with_ev_and_od_vec(buffered_comm_t *a,void **vec,size_t nbytes_per_site)
{
  ...
  //check buffer size matching
  if(a->tot_mess_size!=nbytes_per_site*bord_vol)
    crash("wrong buffer size (%d) for %d border)",a->tot_mess_size,nbytes_per_site*bord_vol);
  ...
  NISSA_PARALLEL_LOOP(ibord_lx,0,bord_vol)
    {
      ...
      memcpy(nissa_send_buf+ibord_lx*nbytes_per_site,(char*)(vec[par])+source_eo*nbytes_per_site,nbytes_per_site);
    }
  ...
  NISSA_PARALLEL_LOOP(ibord_lx,0,bord_vol)
    {
      ...
      memcpy((char*)(vec[par])+dest_eo*nbytes_per_site,nissa_recv_buf+ibord_lx*nbytes_per_site,nbytes_per_site);
    }
  ...
  if(!check_borders_valid(vec[EVN])||!check_borders_valid(vec[ODD]))
    {
      ...
      //fill the communicator buffer, start the communication and take time
      fill_buffered_sending_buf_with_ev_and_od_vec(a,vec,nbytes_per_site);
      buffered_comm_start(a);
      if(IS_MASTER_THREAD) tot_nissa_comm_time+=take_time();
    }
}
  ...
  if(a->comm_in_prog && a->nrequest!=0)
    {
      ...
      //wait communication to finish, fill back the vector and take time
      buffered_comm_wait(a);
      fill_ev_and_od_bord_with_buffered_receiving_buf(vec,a,nbytes_per_site);
      if(IS_MASTER_THREAD) tot_nissa_comm_time+=take_time();
...
{
  buffered_start_communicating_ev_and_od_borders(a,vec,nbytes_per_site);
  buffered_finish_communicating_ev_and_od_borders(vec,a,nbytes_per_site);
}
