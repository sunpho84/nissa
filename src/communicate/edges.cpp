...
// 		    MPI_Irecv(data+rece,1,MPI_EDGES_RECE[iedge],rank_neighup[jdir],imessage,MPI_COMM_WORLD,request+(nrequest++));
// 		    MPI_Isend(data+send,1,MPI_EDGES_SEND[iedge],rank_neighdw[jdir],imessage++,MPI_COMM_WORLD,request+(nrequest++));
		    
// 		    //Send the i-j+ internal edge to the j+ rank as i-j- external edge
// 		    send=(locVol+bord_offset[idir]+pos_edge_offset)*nbytes_per_site;
// 		    rece=(locVol+bord_vol+edge_offset[iedge])*nbytes_per_site;
// 		    MPI_Irecv(data+rece,1,MPI_EDGES_RECE[iedge],rank_neighdw[jdir],imessage,MPI_COMM_WORLD,request+(nrequest++));
// 		    MPI_Isend(data+send,1,MPI_EDGES_SEND[iedge],rank_neighup[jdir],imessage++,MPI_COMM_WORLD,request+(nrequest++));
...
// 			    MPI_Irecv((char*)(data[par])+ext_edge_recv_start,1,MPI_EDGES_RECE[icomm_recv],rank_neigh[!vnu][nu],imessage,MPI_COMM_WORLD,request+(nrequest++));
// 			    MPI_Isend((char*)(data[par])+int_edge_send_start,1,MPI_EDGES_SEND[icomm_send],rank_neigh[vnu][nu],imessage,MPI_COMM_WORLD,request+(nrequest++));
