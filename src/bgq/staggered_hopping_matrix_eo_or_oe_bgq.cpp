...
namespace nissa
{
  ...
  THREADABLE_FUNCTION_5ARG(apply_staggered_hopping_matrix_oe_or_eo_bgq_nocomm_nobarrier, bi_oct_su3**,conf, int,istart, int,iend, bi_color*,in, int,oe_or_eo)
  {
    ...
    bi_color *out=(bi_color*)send_buf;
  ...
  THREADABLE_FUNCTION_0ARG(bgq_staggered_hopping_matrix_oe_or_eo_vdir_VN_comm_and_buff_fill)
  {
    ...
    const int fact=2;
    bi_color *bgq_hopping_matrix_output_data=(bi_color*)send_buf+bord_volh/fact; //half vol bisp = vol sp 
      ...
      NISSA_CHUNK_LOOP(base_isrc,0,vbord_vol/4/fact,thread_in_team_id,nthreads_in_team)
	{
	  ...
	  STORE_REG_BI_COLOR(((bi_color*)send_buf)[idst_buf],to_buf);
	}
  ...
  THREADABLE_FUNCTION_0ARG(bgq_staggered_hopping_matrix_oe_or_eo_vdir_VN_local_transpose)
  {
    ...
    const int fact=2;
    int v=vnode_paral_dir;
    bi_color *bgq_hopping_matrix_output_vdir_buffer=(bi_color*)send_buf+bord_volh/fact+8*loc_volh/fact;
    
    FORM_TWO_THREAD_TEAMS();
    
    //backward scatter (forward derivative)
    bi_color *base_out_bw=(bi_color*)send_buf+(bord_volh+1*8*vdir_bord_vol)/fact;
    ...
    bi_color *base_out_fw=(bi_color*)send_buf+(bord_volh+0*8*vdir_bord_vol)/fact;  
  ...
  THREADABLE_FUNCTION_0ARG(start_staggered_hopping_matrix_oe_or_eo_bgq_communications)
  {
    ...
    if(paral_dir[vnode_paral_dir]) bgq_staggered_hopping_matrix_oe_or_eo_vdir_VN_comm_and_buff_fill();
