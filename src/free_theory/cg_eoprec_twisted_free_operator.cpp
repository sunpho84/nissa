...
namespace nissa
{
  ...
  void inv_tmDkern_eoprec_square_eos(spin *sol,spin *guess,const tm_quark_info& qu,int nitermax,double residue,spin *source)
  {
    ...
    const int64_t n=locVolh()*sizeof(spin)/sizeof(double);
  ...
  void inv_tmD_cg_eoprec_eos(spin *solution_lx,spin *guess_Koo,const tm_quark_info& qu,int nitermax,double residue,spin *source_lx)
  {
	...
	for(int ri=0;ri<2;ri++)
	  { //gamma5 is explicitely wrote
	    varphi[ieo.nastyConvert()][id  ][ri]=+source_eos[ODD][ieo.nastyConvert()][id  ][ri]+varphi[ieo.nastyConvert()][id  ][ri]*0.5;
	    varphi[ieo.nastyConvert()][id+2][ri]=-source_eos[ODD][ieo.nastyConvert()][id+2][ri]-varphi[ieo.nastyConvert()][id+2][ri]*0.5;
	  }
	...
	for(int ri=0;ri<2;ri++)
	  varphi[ieo.nastyConvert()][id][ri]=source_eos[EVN][ieo.nastyConvert()][id][ri]+varphi[ieo.nastyConvert()][id][ri]*0.5;
    NISSA_PARALLEL_LOOP_END;
    set_borders_invalid(varphi);
