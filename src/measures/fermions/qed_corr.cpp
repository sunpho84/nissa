...
namespace nissa
{
  ...
  namespace stag
  {
    ...
    //insert the tadpol
    void insert_tadpole(eo_ptr<color> out,eo_ptr<quad_su3> conf,theory_pars_t* theory_pars,int iflav,eo_ptr<color> in,double* tad,const GlbCoord& t)
    {
    ...
    void insert_external_source(eo_ptr<color> out,eo_ptr<quad_su3> conf,theory_pars_t* theory_pars,int iflav,eo_ptr<spin1field> curr,eo_ptr<color> in,const GlbCoord& t)
    {
    ...
    void insert_time_conserved_vector_current(eo_ptr<color> out,eo_ptr<quad_su3> conf,theory_pars_t* theory_pars,int iflav,eo_ptr<color> in,const GlbCoord& t)
    {
  ...
  void measure_qed_corr(eo_ptr<quad_su3> conf,theory_pars_t theory_pars,qed_corr_meas_pars_t meas_pars,int iconf,int conf_created)
  {
    ...
    // eo_ptr<spin1field> photon_field={nissa_malloc("photon_phi_ev",(locVolh+bordVolh).nastyConvert(),spin1field),nissa_malloc("photon_phi_od",(locVolh+bordVolh).nastyConvert(),spin1field)};
    // eo_ptr<color> M[nprop_t*nflavs];
    // for(int i=0;i<nprop_t*nflavs;i++)
    //   for(int par=0;par<2;par++)
    // 	M[i][par]=nissa_malloc(combine("M_%d_%d",i,par).c_str(),(locVolh+bordVolh).nastyConvert(),color);
