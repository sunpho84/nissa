...
namespace nissa
{
  ...
  void measure_iD_spectrum(color** eigvec,eo_ptr<quad_su3> conf,complex* charge_cut,complex* eigval,int neigs,double eig_precision,int wspace_size)
  {
    ...
    //identity backfield
    eo_ptr<quad_u1> u1b={nissa_malloc("u1b",locVolhWithBord.nastyConvert(),quad_u1),nissa_malloc("u1b",locVolhWithBord.nastyConvert(),quad_u1)};
    init_backfield_to_id(u1b);
    
    //temporary vectors
    eo_ptr<color> tmpvec_eo={nissa_malloc("tmpvec_eo_EVN",locVolhWithBord.nastyConvert(),color),nissa_malloc("tmpvec_eo_ODD",locVolhWithBord.nastyConvert(),color)};
    
    //results of the g5 application
    eo_ptr<color> eigvec_g5_eo={nissa_malloc("eigvec_g5_EVN",locVolhWithBord.nastyConvert(),color),nissa_malloc("eigvec_g5_ODD",locVolhWithBord.nastyConvert(),color)};
	...
	for(int jeig=ieig;jeig<neigs;jeig++)
	  {
	    complex_vector_glb_scalar_prod(charge_cut[ieig*neigs+jeig],(complex*)eigvec[jeig],(complex*)eigvec_g5_lx,locVol.nastyConvert()*sizeof(color)/sizeof(complex));
  ...
  void measure_spectral_proj(FILE *file,spectr_proj_meas_pars_t &meas_pars,quad_su3 *conf_lx,int iconf,int nsmooth)
  {
    ...
    eo_ptr<quad_su3> conf_eo={nissa_malloc("conf_eo_EVN",locVolhWithBordAndEdge.nastyConvert(),quad_su3),nissa_malloc("conf_eo_ODD",locVolhWithBordAndEdge.nastyConvert(),quad_su3)};
