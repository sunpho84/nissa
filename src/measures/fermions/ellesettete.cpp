...
namespace nissa
{
  ...
  void measure_ellesettete(const EoField<quad_su3>& conf,
			   ...
			   const ellesettete_meas_pars_t& meas_pars,
			   const int& iconf,
			   const int& conf_created)
  {
    const int& nflavs=theory_pars.nflavs();
    const int& method=meas_pars.method;
    const double& epsilon=meas_pars.epsilon;
    const GAMMA_INT& DISC_TASTE_CHANNEL=meas_pars.taste_channel;
    const GAMMA_INT CONN_TASTE_CHANNEL=static_cast<GAMMA_INT>(DISC_TASTE_CHANNEL^15);
    
    //open the file, allocate point result and source
    FILE *file=open_file(meas_pars.path,conf_created?"w":"a");
    LxField<complex> point_result("point_result");
    NEW_FIELD_T(source);
		...
		for(int ihit=0;ihit<meas_pars.nhits;ihit++)
		  {
			...
			if(ihit==meas_pars.nhits-1)
			  master_fprintf(file," # Tr_bubble_plus source time %d\n", glb_t);
			SUMM_THE_TIME_TRACE_PRINT_AT_LAST_HIT(Tr_bubble_plus,g5_t_source,PROP_PLUS);
			if(ihit==meas_pars.nhits-1)
			  master_fprintf(file,"\n # Tr_bubble_minus source time %d\n", glb_t);
			SUMM_THE_TIME_TRACE_PRINT_AT_LAST_HIT(Tr_bubble_minus,g5_t_source,PROP_MINUS);
			break;
			...
			if(ihit==meas_pars.nhits-1)
			  master_fprintf(file," # Tr_no_insertion_bubble source time %d\n", glb_t);
