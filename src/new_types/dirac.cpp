...
namespace nissa
{
  ...
  void init_base_gamma()
  {
	...
	spinspin_dirac_summ_the_prod_double(opg[mu],&(base_gamma[igamma_of_mu(Direction(mu))]),+1);
	spinspin_dirac_summ_the_prod_double(omg[mu],&(base_gamma[igamma_of_mu(Direction(mu))]),-1);
      }
  }
}
