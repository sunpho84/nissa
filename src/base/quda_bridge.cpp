...
namespace quda_iface
{
  ...
  void initialize()
  {
	...
	for(int mu=0;mu<NDIM;mu++)
	  quda_conf[mu]=nissa_malloc("quda_conf",locVol.nastyConvert(),su3);
  ...
  void remap_nissa_to_quda(quda_conf_t out,quad_su3 *in)
  {
    ...
    for ( typename OMPThreadsIndices < std :: decay_t < decltype ( 0 ) > , std :: decay_t < decltype ( locVol ) >> :: ForType _ivol = 0 ; _ivol < locVol() ; _ivol ++ ) {
      const typename OMPThreadsIndices<
          std ::decay_t<decltype(0)>,
          std ::decay_t<decltype(locVol)>>::ActualType ivol = _ivol;
      {
