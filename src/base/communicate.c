	...
	if(paral_dir[i]!=0)
	  {
	    ...
	    MPI_Irecv(data+start_lx_bord_rece_up[i]*nbytes_per_site,1,MPI_BORDS_RECE[i],rank_neighup[i],83+i, 
		      cart_comm,&request[nrequest++]);
	    MPI_Isend(data+start_lx_bord_send_up[i]*nbytes_per_site,1,MPI_BORDS_SEND[i],rank_neighdw[i],83+i,
		      cart_comm,&request[nrequest++]);
	    
	    //sending the lower border to the upper node
	    MPI_Irecv(data+start_lx_bord_rece_dw[i]*nbytes_per_site,1,MPI_BORDS_RECE[i],rank_neighdw[i],87+i, 
		      cart_comm,&request[nrequest++]);
	    MPI_Isend(data+start_lx_bord_send_dw[i]*nbytes_per_site,1,MPI_BORDS_SEND[i],rank_neighup[i],87+i,
		      cart_comm,&request[nrequest++]);
	  }
...
{communicate_lx_borders(u,MPI_LX_SU3_BORDS_SEND,MPI_LX_SU3_BORDS_RECE,sizeof(su3));}

//Send the borders of the gauge configuration
void communicate_lx_quad_su3_borders(quad_su3 *conf)
{communicate_lx_borders(conf,MPI_LX_QUAD_SU3_BORDS_SEND,MPI_LX_QUAD_SU3_BORDS_RECE,sizeof(quad_su3));}

//Send the edges: usefuls for hyp
void communicate_lx_su3_edges(su3 *u)
{communicate_lx_edges(u,MPI_LX_SU3_BORDS_SEND,MPI_LX_SU3_BORDS_RECE,MPI_LX_SU3_EDGES_SEND,MPI_LX_SU3_EDGES_RECE,sizeof(su3));}

//Send the edges of the gauge configuration
void communicate_lx_quad_su3_edges(quad_su3 *conf)
{communicate_lx_edges(conf,MPI_LX_QUAD_SU3_BORDS_SEND,MPI_LX_QUAD_SU3_BORDS_RECE,MPI_LX_QUAD_SU3_EDGES_SEND,MPI_LX_QUAD_SU3_EDGES_RECE,sizeof(quad_su3));}

//Send the borders of a spincolor vector
void communicate_lx_spincolor_borders(spincolor *s)
{communicate_lx_borders(s,MPI_LX_SPINCOLOR_BORDS_SEND,MPI_LX_SPINCOLOR_BORDS_RECE,sizeof(spincolor));}

//Send the borders of a spincolor_128 vector
void communicate_lx_spincolor_128_borders(spincolor_128 *s)
{communicate_lx_borders(s,MPI_LX_SPINCOLOR_128_BORDS_SEND,MPI_LX_SPINCOLOR_128_BORDS_RECE,sizeof(spincolor_128));}
...
void start_communicating_ev_borders(int *nrequest_ret,MPI_Request *request,void *ev_data,MPI_Datatype *MPI_EV_BORDS_SEND_TXY,MPI_Datatype *MPI_EV_BORDS_SEND_Z,MPI_Datatype *MPI_EV_BORDS_RECE,int nbytes_per_site)
{
  ...
  if(!check_borders_valid(ev_data))
	...
	if(paral_dir[mu]!=0)
	  {
	    //sending the upper border to the lower node
	    MPI_Irecv(ev_data+start_eo_bord_rece_up[mu]*nbytes_per_site,1,MPI_EV_BORDS_RECE[mu],rank_neighup[mu],83+mu,cart_comm,&request[nrequest++]);
	    MPI_Isend(ev_data+start_eo_bord_send_up[mu]*nbytes_per_site,1,MPI_EV_BORDS_SEND_TXY[mu],rank_neighdw[mu],83+mu,cart_comm,&request[nrequest++]);
	    
	    //sending the lower border to the upper node
	    MPI_Irecv(ev_data+start_eo_bord_rece_dw[mu]*nbytes_per_site,1,MPI_EV_BORDS_RECE[mu],rank_neighdw[mu],91+mu,cart_comm,&request[nrequest++]);
	    MPI_Isend(ev_data+start_eo_bord_send_dw[mu]*nbytes_per_site,1,MPI_EV_BORDS_SEND_TXY[mu],rank_neighup[mu],91+mu,cart_comm,&request[nrequest++]);
	  }
      
      if(paral_dir[3]!=0)
	{
	  //sending the upper border to the lower node
	  MPI_Irecv(ev_data+start_eo_bord_rece_up[3]*nbytes_per_site,1,MPI_EV_BORDS_RECE[3],rank_neighup[3],86,cart_comm,&request[nrequest++]);
	  MPI_Isend(ev_data,1,MPI_EV_BORDS_SEND_Z[0],rank_neighdw[3],86,cart_comm,&request[nrequest++]);
...
void communicate_od_borders(void *od_data,MPI_Datatype *MPI_EV_BORDS_SEND_TXY,MPI_Datatype *MPI_OD_BORDS_SEND_Z,MPI_Datatype *MPI_EV_BORDS_RECE,int nbytes_per_site)
{
  ...
  if(!check_borders_valid(od_data))
	...
	if(paral_dir[mu]!=0)
	  {
	    //sending the upper border to the lower node
	    MPI_Irecv(od_data+start_eo_bord_rece_up[mu]*nbytes_per_site,1,MPI_EV_BORDS_RECE[mu],rank_neighup[mu],83+mu,cart_comm,&request[nrequest++]);
	    MPI_Isend(od_data+start_eo_bord_send_up[mu]*nbytes_per_site,1,MPI_EV_BORDS_SEND_TXY[mu],rank_neighdw[mu],83+mu,cart_comm,&request[nrequest++]);
	    
	    //sending the lower border to the upper node
	    MPI_Irecv(od_data+start_eo_bord_rece_dw[mu]*nbytes_per_site,1,MPI_EV_BORDS_RECE[mu],rank_neighdw[mu],91+mu,cart_comm,&request[nrequest++]);
	    MPI_Isend(od_data+start_eo_bord_send_dw[mu]*nbytes_per_site,1,MPI_EV_BORDS_SEND_TXY[mu],rank_neighup[mu],91+mu,cart_comm,&request[nrequest++]);
	  }
      
      if(paral_dir[3]!=0)
	{
	  //sending the upper border to the lower node
	  MPI_Irecv(od_data+start_eo_bord_rece_up[3]*nbytes_per_site,1,MPI_EV_BORDS_RECE[3],rank_neighup[3],86,cart_comm,&request[nrequest++]);
	  MPI_Isend(od_data,1,MPI_OD_BORDS_SEND_Z[0],rank_neighdw[3],86,cart_comm,&request[nrequest++]);
		    
	  //sending the lower border to the upper node
	  MPI_Irecv(od_data+start_eo_bord_rece_dw[3]*nbytes_per_site,1,MPI_EV_BORDS_RECE[3],rank_neighdw[3],94,cart_comm,&request[nrequest++]);
	  MPI_Isend(od_data,1,MPI_OD_BORDS_SEND_Z[1],rank_neighup[3],94,cart_comm,&request[nrequest++]);
	}
...
void communicate_eo_borders(void **data,MPI_Datatype *MPI_EO_BORDS_SEND_TXY,MPI_Datatype *MPI_EV_BORDS_SEND_Z,MPI_Datatype *MPI_OD_BORDS_SEND_Z,MPI_Datatype *MPI_EO_BORDS_RECE,int nbytes_per_site)
{
  ...
  for(int par=0;par<2;par++)
	...
	for(int mu=0;mu<3;mu++) //0,1,2
	  if(paral_dir[mu]!=0)
	    {
	      //sending the upper border to the lower node
	      MPI_Irecv(data[par]+start_eo_bord_rece_up[mu]*nbytes_per_site,1,MPI_EO_BORDS_RECE[mu],rank_neighup[mu],83+par*4+mu,
			cart_comm,&request[nrequest++]);
	      MPI_Isend(data[par]+start_eo_bord_send_up[mu]*nbytes_per_site,1,MPI_EO_BORDS_SEND_TXY[mu],rank_neighdw[mu],83+par*4+mu,
			cart_comm,&request[nrequest++]);
	      
	      //sending the lower border to the upper node
	      MPI_Irecv(data[par]+start_eo_bord_rece_dw[mu]*nbytes_per_site,1,MPI_EO_BORDS_RECE[mu],rank_neighdw[mu],91+par*4+mu, 
			cart_comm,&request[nrequest++]);
	      MPI_Isend(data[par]+start_eo_bord_send_dw[mu]*nbytes_per_site,1,MPI_EO_BORDS_SEND_TXY[mu],rank_neighup[mu],91+par*4+mu,
			cart_comm,&request[nrequest++]);
	    }
    }
  ...
  if(paral_dir[3]!=0)
	...
	{
	  //sending the upper border to the lower node
	  MPI_Irecv(data[EVN]+start_eo_bord_rece_up[3]*nbytes_per_site,1,MPI_EO_BORDS_RECE[3],rank_neighup[3],86,cart_comm,&request[nrequest++]);
	  MPI_Isend(data[EVN],1,MPI_EV_BORDS_SEND_Z[0],rank_neighdw[3],86,cart_comm,&request[nrequest++]);
	  
	  //sending the lower border to the upper node
	  MPI_Irecv(data[EVN]+start_eo_bord_rece_dw[3]*nbytes_per_site,1,MPI_EO_BORDS_RECE[3],rank_neighdw[3],94,cart_comm,&request[nrequest++]);
	  MPI_Isend(data[EVN],1,MPI_EV_BORDS_SEND_Z[1],rank_neighup[3],94,cart_comm,&request[nrequest++]);
	}
      
      if(!check_borders_valid(data[ODD]))
	{
	  //sending the upper border to the lower node
	  MPI_Irecv(data[ODD]+start_eo_bord_rece_up[3]*nbytes_per_site,1,MPI_EO_BORDS_RECE[3],rank_neighup[3],90,cart_comm,&request[nrequest++]);
	  MPI_Isend(data[ODD],1,MPI_OD_BORDS_SEND_Z[0],rank_neighdw[3],90,cart_comm,&request[nrequest++]);
	  
	  //sending the lower border to the upper node
	  MPI_Irecv(data[ODD]+start_eo_bord_rece_dw[3]*nbytes_per_site,1,MPI_EO_BORDS_RECE[3],rank_neighdw[3],98,cart_comm,&request[nrequest++]);
	  MPI_Isend(data[ODD],1,MPI_OD_BORDS_SEND_Z[1],rank_neighup[3],98,cart_comm,&request[nrequest++]);
	}
    }
...
{communicate_eo_borders((void**)eo_conf,MPI_EO_QUAD_SU3_BORDS_SEND_TXY,MPI_EV_QUAD_SU3_BORDS_SEND_Z,MPI_OD_QUAD_SU3_BORDS_SEND_Z,MPI_EO_QUAD_SU3_BORDS_RECE,sizeof(quad_su3));}

//Send the borders of an even color
void communicate_ev_color_borders(color *ev)
{communicate_ev_borders(ev,MPI_EO_COLOR_BORDS_SEND_TXY,MPI_EV_COLOR_BORDS_SEND_Z,MPI_EO_COLOR_BORDS_RECE,sizeof(color));}
		  ...
		  if(paral_dir[mu] && paral_dir[nu])
		    {
		      ...
		      MPI_Irecv(data[par]+ext_edge_recv_start,1,MPI_EDGES_RECE[icomm_recv],rank_neigh[!vnu][nu],83+imessage,
				cart_comm,&request[nrequest++]);
		      MPI_Isend(data[par]+int_edge_send_start,1,MPI_EDGES_SEND[icomm_send],rank_neigh[vnu][nu],83+imessage,
				cart_comm,&request[nrequest++]);
...
{communicate_eo_edges((void**)conf,MPI_EO_QUAD_SU3_BORDS_SEND_TXY,MPI_EV_QUAD_SU3_BORDS_SEND_Z,MPI_OD_QUAD_SU3_BORDS_SEND_Z,MPI_EO_QUAD_SU3_BORDS_RECE,MPI_EO_QUAD_SU3_EDGES_SEND,MPI_EO_QUAD_SU3_EDGES_RECE,sizeof(quad_su3));}
