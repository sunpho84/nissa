...
#define DO8(buf) DO4(buf);DO4(buf);
  ...
  namespace checksum_getter
  {
    ...
    template <typename T>
    CUDA_HOST_AND_DEVICE
    auto get_data(const T* data,const LocLxSite& ivol,const size_t bps=sizeof(T))
    {
    ...
    template <typename T>
    CUDA_HOST_AND_DEVICE
    auto get_data(const eo_ptr<T>& data,const LocLxSite& ivol,const size_t bps=sizeof(T))
    {
      const Parity eo=loclx_parity(ivol);
      const LocEoSite loceo=loceo_of_loclx(ivol);
  ...
  void checksum_compute_nissa_data(checksum& check,const T& data,int prec,const size_t bps)
  {
    ...
    NISSA_PARALLEL_LOOP(ivol,0,locVol)
      {
	uint32_t ildg_ivol=glbCoordOfLoclx(ivol,Dir(0))();
	for(Dir mu=NDIM-1;mu>0;mu--) ildg_ivol=ildg_ivol*glbSize(mu)()+glbCoordOfLoclx(ivol,mu)();
