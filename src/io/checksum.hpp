...
#define DO8(buf) DO4(buf);DO4(buf);
  ...
  {
    constexpr int bps=sizeof(T);
    
    if(little_endian)
      {
	unsigned char tmp[bps];
  ...
  template <typename T>
  void checksum_compute_nissa_data(checksum& check,
				   const LxField<T>& data,
				   const int& prec)
  {
	...
	ivol,
	{
	  ...
	  typename LxField<T>::Fund tmp[LxField<T>::nInternalDegs];
	  for(int iDeg=0;iDeg<(LxField<T>::nInternalDegs);iDeg++)
	    tmp[iDeg]=data(ivol,iDeg);
	  
	  const uint32_t temp=ildg_crc32_fix_endianness(0,(char*)tmp,prec);
	  
	  for(int i=0;i<2;i++)
	    buff[ivol][i]=temp<<crc_rank[i]|temp>>(32-crc_rank[i]);
	});
