...
namespace nissa
{
  ...
  template <class T> void write_double_vector(ILDG_File &file,T *data,size_t nbits,const char *header_message,ILDG_message *mess=NULL)
  {
  ...
  template <class T> void write_double_vector(std::string path,T *data,size_t nbits,const char *header_message,ILDG_message *mess=NULL)
  {
    ...
    write_double_vector(file,data,nbits,header_message,mess);
