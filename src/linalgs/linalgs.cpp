...
void reduce(int size, int threads, int blocks, int whichKernel, T *d_idata,
  ...
  THREADABLE_FUNCTION_4ARG(double_vector_glb_scalar_prod, double*,glb_res, double*,a, double*,b, int,n)
  {
    ...
    const int block_dimension(NUM_THREADS);
    const int grid_dimension((n+block_dimension-1)/block_dimension);
    
    reduce<double>(n,NUM_THREADS,grid_dimension,6,reducing_buffer,reducing_buffer);
