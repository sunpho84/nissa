...
namespace nissa
{
  ...
  THREADABLE_FUNCTION_4ARG(apply_overlap_kernel, spincolor*,out, quad_su3*,conf, double,M, spincolor*,in)
  {
	...
	for(int c=0;c<3;c++)
	  {
	    out[X][0][c][0]=-0.5*out[X][0][c][0]+kcf*in[X][0][c][0]; out[X][0][c][0]+=M*in[X][0][c][0];
	    out[X][0][c][1]=-0.5*out[X][0][c][1]+kcf*in[X][0][c][1]; out[X][0][c][1]+=M*in[X][0][c][1];
	    out[X][1][c][0]=-0.5*out[X][1][c][0]+kcf*in[X][1][c][0]; out[X][1][c][0]+=M*in[X][1][c][0];
	    out[X][1][c][1]=-0.5*out[X][1][c][1]+kcf*in[X][1][c][1]; out[X][1][c][1]+=M*in[X][1][c][1];
	    out[X][2][c][0]=+0.5*out[X][2][c][0]-kcf*in[X][2][c][0]; out[X][2][c][0]+=M*in[X][2][c][0];
	    out[X][2][c][1]=+0.5*out[X][2][c][1]-kcf*in[X][2][c][1]; out[X][2][c][1]+=M*in[X][2][c][1];
	    out[X][3][c][0]=+0.5*out[X][3][c][0]-kcf*in[X][3][c][0]; out[X][3][c][0]+=M*in[X][3][c][0];
	    out[X][3][c][1]=+0.5*out[X][3][c][1]-kcf*in[X][3][c][1]; out[X][3][c][1]+=M*in[X][3][c][1];
	  }
      }
