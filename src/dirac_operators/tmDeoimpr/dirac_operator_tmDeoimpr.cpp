...
namespace nissa
{
  ...
  THREADABLE_FUNCTION_7ARG(tmD_or_Qkern_eoprec_eos, spincolor*,out, spincolor*,temp, quad_su3**,conf, double,kappa, double,mu, spincolor*, in, int,D_or_Q)
  {
    ...
    //gamma5 sign is explicitely implemented according to the OPPOSITE of flag (it's present in D, not in Q)
    double g5_sign=(D_or_Q?1:-1);
	  ...
	  for(int ri=0;ri<2;ri++)
	    {
	      ...
	      out[ivol][id+2][ic][ri]=g5_sign*(-temp[ivol][id+2][ic][ri]+out[ivol][id+2][ic][ri]*0.25);
	    }
