...
double fun_fit_phiD(double A,double B,double C,double ml,double a)
{
  ...
  double f=f0.data[iboot];
  double cl_g=-3.0/4*(1+3.0*g*g)*m2pi/sqr(4*M_PI*f)*log(m2pi);
  double cl_h1=-3.0/4*(h*h)*m2pi*m2pi/sqr(4*M_PI*f)/(4*delta*delta)*log(m2pi);
  double cl_h2=3.0/2*h*m2pi*m2pi/sqr(4*M_PI*f)/(4*delta*delta)*log(m2pi)*0.7;
  
  //cl_h1=cl_h2=0;
  
  return A * ( 1 + cl_g + cl_h1 + cl_h2 + B*m2pi + C*a*a );
}
