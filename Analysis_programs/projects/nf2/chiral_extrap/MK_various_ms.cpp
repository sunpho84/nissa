...
double fun_fit_M2K(double A,double B,double C,double ml,double a)
{return A + B*ml + C*a*a;}
...
double chi2(double A,double B,double C,double *a)
{
  ...
  for(int iens=0;iens<nens;iens++)
    {
      double ch2_M2K_term=pow((Y_fit[iens]-fun_fit_M2K(A,B,C,X_fit[iens],a[ibeta[iens]]))/err_Y_fit[iens],2);
      ch2+=ch2_M2K_term;//+ch2_M2K_term;
    }
...
int main(int narg,char **arg)
{
  ...
  //load data
  M2K=bvec(nens,nboot,njack);

  bvec MK_chir_cont(3,nboot,njack);
  bvec M2K_chir_cont(3,nboot,njack);
  for(int iref=0;iref<3;iref++)
    {
      M2K.load(meson_mass_file,iref);
      ...
      boot M2K_chir[nbeta];
      bvec M2K_estr_ml(nbeta,nboot,njack);
      for(int ib=0;ib<nbeta;ib++)
	{
	  ...
	  for(int iboot=0;iboot <nboot+1;iboot++)
	    {
	      ...
	      M2K_chir_cont[iref].data[iboot]=fun_fit_M2K(A[iboot],B[iboot],C[iboot],ml_phys[iboot],0);
	      MK_chir_cont[iref].data[iboot]=sqrt(M2K_chir_cont[iref].data[iboot]);
	      M2K_chir[ib].data[iboot]=fun_fit_M2K(A[iboot],B[iboot],C[iboot],ml_phys[iboot],lat[ib][iboot]);
	      
	      if(r!=-1) M2K_estr_ml.data[ib].data[iboot]=M2K[r][iboot]*fun_fit_M2K(A[iboot],B[iboot],C[iboot],ml_phys[iboot],0)/fun_fit_M2K(A[iboot],B[iboot],C[iboot],ml[r][iboot],0);
	    }
	}
      
      //chiral and continuum
      cout<<"MK = ("<<MK_chir_cont[iref]*1000<<") MeV"<<endl;
      
      par_res_fit_M2K=bvec(3,nboot,njack);
      ...
      plot_funz_ml(combine("M2K_funz_ml_%d.xmg",iref).c_str(),meson_name,tag_ml,meson_name,ml,M2K,par_res_fit_M2K,ml_phys.med(),fun_fit_M2K,M2K_chir_cont[iref]);
      plot_funz_a2(combine("M2K_funz_a2_%d.xmg",iref).c_str(),meson_name,tag_a2,meson_name,lat_med_fm,M2K_estr_ml,par_res_fit_M2K,fun_fit_M2K,M2K_chir_cont[iref]);
    }
