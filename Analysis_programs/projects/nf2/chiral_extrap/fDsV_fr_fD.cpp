  ...
  if(fun!=NULL)
      ...
      for(int ib=0;ib<nbeta;ib++)
	{
	  ...
	  for(int ipoint=0;ipoint<npoint;ipoint++)
	    for(iboot=plot_iboot=0;iboot<nboot+1;plot_iboot=iboot++)
...
int main(int narg,char **arg)
{
  ...
  boot U_chir[nbeta],U_chir_cont(nboot,njack);
  bvec U_estr_ml(nbeta,nboot,njack);
  for(int ib=0;ib<nbeta;ib++)
    {
      U_chir[ib]=boot(nboot,njack);
      for(iboot=0;iboot <nboot+1;iboot++)
	{
  ...
  boot mD_sV(nboot,njack);
  mD_sV.load("../../../VKVK/Ds/chiral_cont_extrapol_M/MD_s",0);
  cout<<"MD_sV: "<<mD_sV<<endl;

  //chiral and continuum
  cout<<"PhiDs*/PhiD = ("<<U_chir_cont<<")"<<endl;
  U_chir_cont.write_to_binfile("phiDsV_fr_phiD");
  
  boot x=U_chir_cont/sqrt(mD_sV/mD_phys);
  cout<<"fDs*/fD = ("<<x<<")"<<endl;
  U_chir_cont.write_to_binfile("fDsV_fr_fD");
