...
bvec ratio;
boot ghat;
double ghat_ave=0.61,ghat_err=0.07;

bvec ml;
int ref_ml_beta[4]={-1,-1,-1,-1};

bvec par_res_fit_ratio;
...
double fun_fit_ratio(double A,double B,double C,double ml,double a)
{
  double xi=db0.data[iboot]*ml/sqr(4*M_PI*f0.data[iboot]);
  double g=ghat.data[iboot];
  double cl=( -2+9.0*sqr(g) )/4.0*xi*log(xi);
  
  return A * ( 1 + cl + B*xi + C*a*a );
}
...
void plot_funz_ml(const char *out_path,const char *title,const char *xlab,const char *ylab,bvec &X,bvec &Y,bvec &par,double X_phys,double (*fun)(double,double,double,double,double),boot &chiral_extrap_cont)
{
  ...
  for(int iens=0;iens<nens;iens++) cout<<iens<<" "<<ibeta[iens]<<" "<<X[iens]<<" "<<Y[iens]<<endl;
...
double chi2(double A,double B,double C,double *a)
{
  ...
  for(int iens=0;iens<nens;iens++)
    {
      double ch2_ratio_term=pow((Y_fit[iens]-fun_fit_ratio(A,B,C,X_fit[iens],a[ibeta[iens]]))/err_Y_fit[iens],2);
      ch2+=ch2_ratio_term;
      if(fine) cout<<iens<<" "<<mass[iens][iml_un[iens]]<<" "<<Y_fit[iens]<<" "<<fun_fit_ratio(A,B,C,X_fit[iens],a[ibeta[iens]])<<" "<<ch2_ratio_term<<" "<<err_Y_fit[iens]<<endl;
    }
...
void fit(boot &A,boot &B,boot &C,bvec &X,bvec &Y)
{
  ...
  minu.DefineParameter(1,"B",0.7,0.0001,0,0);
  minu.DefineParameter(2,"C",0.1,0.0001,0,0);
  ...
  for(iboot=0;iboot<nboot+1;iboot++)
    {
      ...
      minu.DefineParameter(3,"a380",lat[0][iboot],0.001,0,0);
      minu.DefineParameter(4,"a390",lat[1][iboot],0.001,0,0);
      minu.DefineParameter(5,"a405",lat[2][iboot],0.001,0,0);
      minu.DefineParameter(6,"a420",lat[3][iboot],0.001,0,0);
      minu.FixParameter(3);
      minu.FixParameter(4);
      minu.FixParameter(5);
      minu.FixParameter(6);
      ...
      if(iboot==nboot){fine=true;C2=chi2(A.data[iboot],B[iboot],C[iboot],lat_med);fine=false;}
    }
...
int main(int narg,char **arg)
{
  ...
  //read ensemble list, meson ratio and meson name
  ...
  read_formatted_from_file_expecting(meson_phi_file,an_input_file,"%s","meson_ratio_file");
  ...
  //load data
  ratio=bvec(nens,nboot,njack);
  ratio.load(meson_phi_file,0);  
  ...
  boot ratio_chir[nbeta],ratio_chir_cont(nboot,njack);
  bvec ratio_estr_ml(nbeta,nboot,njack),fD_estr_ml(nbeta,nboot,njack);
  for(int ib=0;ib<nbeta;ib++)
    {
      ratio_chir[ib]=boot(nboot,njack);
      for(iboot=0;iboot<nboot+1;iboot++)
	{
	  int r=ref_ml_beta[ib];
	  ratio_chir_cont.data[iboot]=fun_fit_ratio(A[iboot],B[iboot],C[iboot],ml_phys[iboot],0);
	  ratio_chir[ib].data[iboot]=fun_fit_ratio(A[iboot],B[iboot],C[iboot],ml_phys[iboot],lat[ib][iboot]);
	  
	  if(r!=-1) ratio_estr_ml.data[ib].data[iboot]=ratio[r][iboot]*fun_fit_ratio(A[iboot],B[iboot],C[iboot],ml_phys[iboot],0)/fun_fit_ratio(A[iboot],B[iboot],C[iboot],ml[r][iboot],0);
	}
    }
  
  //chiral and continuum
  cout<<"ratio = ("<<ratio_chir_cont/sqrt(mD_s_phys/mD_phys)<<") MeV"<<endl;
  (ratio_chir_cont/sqrt(mD_s_phys/mD_phys)).write_to_binfile("results_ratio_f");
  
  par_res_fit_ratio=bvec(3,nboot,njack);
  
  par_res_fit_ratio.data[0]=A;
  par_res_fit_ratio.data[1]=B;
  par_res_fit_ratio.data[2]=C;
  ...
  plot_funz_ml("ratio_funz_ml.xmg",meson_name,tag_ml,meson_name,ml,ratio,par_res_fit_ratio,ml_phys.med(),fun_fit_ratio,ratio_chir_cont);
  plot_funz_a2("ratio_funz_a2.xmg",meson_name,tag_a2,meson_name,lat_med_fm,ratio_estr_ml,par_res_fit_ratio,fun_fit_ratio,ratio_chir_cont);
