...
double fun_fit_phiDs(double A,double B,double C,double ml,double a)
{
  return A * ( 1 + B*ml + C*a*a );
}

double fun_fit_fDs(double A,double B,double C,double ml,double a)
{
  return A * ( 1 + B*ml + C*a*a );
}
...
double chi2(double A,double B,double C,double *a)
{
  ...
  for(int iens=0;iens<nens;iens++)
    {
      double ch2_phiDs_term=pow((Y_fit[iens]-fun_fit_phiDs(A,B,C,X_fit[iens],a[ibeta[iens]]))/err_Y_fit[iens],2);
      ch2+=ch2_phiDs_term;
    }
...
int main(int narg,char **arg)
{
  ...
  //read ensemble list, meson phiDs and meson name
  ...
  //load data
  phiDs=bvec(nens,nboot,njack);
  phiDs.load(meson_phi_file,0);  
  
  bvec MDs(nens,nboot,njack);
  MDs.load("../cs_interpolation/interpolated_M_Ds",0);
  
  fDs=phiDs/sqrt(MDs);
  
  //load the mDsV from extrapolation
  boot mDsV(nboot,njack);
  mDsV.load("../chiral_cont_extrapol_M/MD_s",0);  
  cout<<"MDsV: "<<mDsV<<endl;
  ...
  boot phiDs_chir[nbeta],phiDs_chir_cont(nboot,njack);
  bvec phiDs_estr_ml(nbeta,nboot,njack),fDs_estr_ml(nbeta,nboot,njack);
  for(int ib=0;ib<nbeta;ib++)
    {
      phiDs_chir[ib]=boot(nboot,njack);
      for(int iboot=0;iboot<nboot+1;iboot++)
	{
	  int r=ref_ml_beta[ib];
	  phiDs_chir_cont.data[iboot]=fun_fit_phiDs(A[iboot],B[iboot],C[iboot],ml_phys[iboot],0);
	  phiDs_chir[ib].data[iboot]=fun_fit_phiDs(A[iboot],B[iboot],C[iboot],ml_phys[iboot],lat[ib][iboot]);
	  
	  if(r!=-1) phiDs_estr_ml.data[ib].data[iboot]=phiDs[r][iboot]*fun_fit_phiDs(A[iboot],B[iboot],C[iboot],ml_phys[iboot],0)/fun_fit_phiDs(A[iboot],B[iboot],C[iboot],ml[r][iboot],0);
	}
    }
  boot fDs_chir_cont=phiDs_chir_cont/sqrt(mDsV);
  
  //chiral and continuum
  cout<<"phiDs = ("<<phiDs_chir_cont*1000<<") MeV"<<endl;
  cout<<"fDs = ("<<fDs_chir_cont*1000<<") MeV"<<endl;
  phiDs_chir_cont.write_to_binfile("results_phiDs");
  fDs_chir_cont.write_to_binfile("results_fDs");
  
  par_res_fit_phiDs=bvec(3,nboot,njack);
  
  par_res_fit_phiDs.data[0]=A;
  par_res_fit_phiDs.data[1]=B;
  par_res_fit_phiDs.data[2]=C;
  ...
  plot_funz_ml("phiDs_funz_ml.xmg",meson_name,tag_ml,meson_name,ml,phiDs,par_res_fit_phiDs,ml_phys.med(),fun_fit_phiDs,phiDs_chir_cont);
  plot_funz_ml("fDs_funz_ml.xmg",meson_name,tag_ml,meson_name,ml,fDs,par_res_fit_phiDs,ml_phys.med(),fun_fit_fDs,fDs_chir_cont);
  plot_funz_a2("phiDs_funz_a2.xmg",meson_name,tag_a2,meson_name,lat_med_fm,phiDs_estr_ml,par_res_fit_phiDs,fun_fit_phiDs,phiDs_chir_cont);
  plot_funz_a2("fDs_funz_a2.xmg",meson_name,tag_a2,meson_name,lat_med_fm,fDs_estr_ml,par_res_fit_phiDs,fun_fit_fDs,fDs_chir_cont);
