...
double fun_fit_W(double A,double B,double C,double D,double ml,double a)
{
  return A * ml * ( 1 + ml*(B+D*log(ml)) + C*a*a );
}
...
double chi2(double A,double B,double C,double D,double *a)
{
  ...
  for(int iens=0;iens<nens;iens++)
    {
      double ch2_W_term=pow((Y_fit[iens]-fun_fit_W(A,B,C,D,X_fit[iens],a[ibeta[iens]]))/err_Y_fit[iens],2);
      ch2+=ch2_W_term;
    }
...
void fit(boot &A,boot &B,boot &C,boot &D,bvec &X,bvec &Y)
{
  ...
  cout<<"Chi2 = "<<C2<<" / "<<nens-4<<" = "<<C2/(nens-4)<<endl;
...
int main(int narg,char **arg)
{
  ...
  char ens_list_path[1024],meson_f_file[1024],meson_M_file[1024],meson_name[1024];
  read_formatted_from_file_expecting(ens_list_path,an_input_file,"%s","ens_list_path");
  read_formatted_from_file_expecting(meson_f_file,an_input_file,"%s","meson_f_file");
  read_formatted_from_file_expecting(meson_M_file,an_input_file,"%s","meson_M_file");
  ...
  M=bvec(nens,nboot,njack);
  f.load(meson_f_file,0);
  M.load(meson_M_file,0);
  W=sqrt(f)*M*M;
  ...
  boot W_chir[nbeta],W_chir_cont(nboot,njack);
  bvec W_estr_ml(nbeta,nboot,njack);
  for(int ib=0;ib<nbeta;ib++)
    {
      W_chir[ib]=boot(nboot,njack);
      for(iboot=0;iboot<nboot+1;iboot++)
	{
	  int r=ref_ml_beta[ib];
	  W_chir_cont.data[iboot]=fun_fit_W(A[iboot],B[iboot],C[iboot],D[iboot],ml_phys[iboot],0);
	  W_chir[ib].data[iboot]=fun_fit_W(A[iboot],B[iboot],C[iboot],D[iboot],ml_phys[iboot],lat[ib][iboot]);
	  
	  if(r!=-1) W_estr_ml.data[ib].data[iboot]=W[r][iboot]*fun_fit_W(A[iboot],B[iboot],C[iboot],D[iboot],ml_phys[iboot],0)/fun_fit_W(A[iboot],B[iboot],C[iboot],D[iboot],ml[r][iboot],0);
	}
    }
  
  //chiral and continuum
  cout<<"W = ("<<W_chir_cont<<") GeV^2.5"<<endl;
  W_chir_cont.write_to_binfile("results_W");
  
  par_res_fit_W=bvec(4,nboot,njack);
  
  par_res_fit_W.data[0]=A;
  par_res_fit_W.data[1]=B;
  par_res_fit_W.data[2]=C;
  par_res_fit_W.data[3]=D;
  ...
  plot_funz_ml("W_funz_ml.xmg",meson_name,tag_ml,meson_name,ml,W,par_res_fit_W,ml_phys.med(),fun_fit_W,W_chir_cont);
  plot_funz_a2("W_funz_a2.xmg",meson_name,tag_a2,meson_name,lat_med_fm,W_estr_ml,par_res_fit_W,fun_fit_W,W_chir_cont);
  
  cout<<W_chir_cont-sqrt(fPi_phys)*sqr(mPi_phys)<<endl;
  cout<<A<<" "<<db0*sqrt(f0)<<endl;
  cout<<A<<" - "<<db0*sqrt(f0)<<" = "<<A-db0*sqrt(f0)<<endl;
