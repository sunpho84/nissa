...
int main()
{
  ...
  FILE *fin=open_file(combine("%s/%s",base_path,"VKVK").c_str(),"r");
  ...
  for(int ims=0;ims<nmass;ims++)
	...
	boot_from_jack(afD12.data[icombo],tempf/tempm,ibeta);
	afD12.data[icombo]*=Za[ibeta];
  ...
  fDs.write_to_binfile("results_fDs");
