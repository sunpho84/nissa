...
int main()
{
  ...
  for(int iens=0;iens<nens;iens++)
	...
	for(int imc=ims;imc<nmass[iens];imc++)
      ...
      f*=sqrt(aM[iens]/lat[b]);
      fV*=sqrt(aMV[iens]/lat[b]);
      x[iens]=fV;
	...
	for(int imc=ims;imc<nmass[iens];imc++)
	  {
	    int ic=icombo(ims,imc,nmass[iens],nlights[iens],mode);
	    int ic_un=icombo(iml_un[iens],imc,nmass[iens],nlights[iens],mode);
	    x[iens][ic]/=f[ic_un];
	    //cout<<ic<<" "<<ic_un<<" "<<fV[ic]<<" "<<f[ic_un]<<" "<<x[iens][ic]<<endl;
	  }
    }
  ...
  fint.write_to_binfile("interpolated_phi");
