	  ...
	  for(int iboot=0;iboot <nboot+1;iboot++)
	    {
	      ...
	      if(r!=-1)
		Mh_estr_ml.data[ib].data[iboot]=Mh[r][iboot]*fun_fit_Mh(A[nboot],B[nboot],C[nboot],D[nboot],ml_phys[nboot],0)/fun_fit_Mh(A[nboot],B[nboot],C[nboot],D[nboot],ml[r][nboot],0);
	    }
	}
