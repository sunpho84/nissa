...
int main()
{
  ...
  bvec af[nens],aphi[nens],ratio[nens];
	...
	for(int imh=0;imh<nmass[iens];imh++)
	  {
	    ...
	    ratio[iens][ic]=aphi[iens][ic]/aphi[iens][imh];
	    cout<<iens<<" "<<iml<<" "<<imh<<" f: "<<af[iens][ic]/lat[ibeta[iens]]<<" phi: "<<ratio[iens][ic]<<endl;
	  }
    }
	  ...
	  for(int iml=1;iml<nlights[iens];iml++)
	    {
	      ...
	      yto[iml-1]=ratio[iens][iml*nmass[iens]+ih];
